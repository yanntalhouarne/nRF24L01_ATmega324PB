...
void nRF_config()
{
	...
	nRF_configReg(RX_PW_P0, 0x00); // Auto-ACK pipe ...
	nRF_configReg(RX_PW_P1, mirf_PAYLOAD); // Data payload pipe
	nRF_configReg(RX_PW_P2, 0x00);
	nRF_configReg(RX_PW_P3, 0x00);
	nRF_configReg(RX_PW_P4, 0x00);
	nRF_configReg(RX_PW_P5, 0x00);
   
    // 250 kbps, TX gain: 0dbm
   	nRF_configReg(RF_SETUP, (0x03<<RF_PWR) | (1<<RF_DR_LOW) | (0<<RF_DR_HIGH));
	   
    // Auto Acknowledgment
    nRF_configReg(EN_AA,(1<<ENAA_P0)|(1<<ENAA_P1)|(0<<ENAA_P2)|(0<<ENAA_P3)|(0<<ENAA_P4)|(0<<ENAA_P5)); // AA enable on Pipe 0
 ...
 *																							*
 ********************************************************************************************/
void nRF_getData(char *data)
{
	nRF_CLEAR_CSN;                                  // Pull down chip select
	SPI1_sendChar(R_RX_PAYLOAD);              // Send cmd to read rx payload
	SPI1_exchangeBytes(data, data, mirf_PAYLOAD); // Read payload
	nRF_SET_CSN;                                  // Pull up chip select
